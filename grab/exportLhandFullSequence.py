...
def export_sequence(cfg, logger=None):
    ...
    lh_mesh_fn = os.path.join(grab_path, '..', seq_data.lhand.vtemp)
    lh_mesh = Mesh(filename=lh_mesh_fn)

    lh_f = lh_mesh.faces
    lh_vtemp = lh_mesh.vertices

    # Extract hand joint rest position

    rest_hand_global_orient = np.zeros((1, seq_data.lhand.params.global_orient.shape[1]))
    rest_hand_transl = np.zeros((1, seq_data.lhand.params.transl.shape[1]))
    rest_hand_pose = np.zeros((1, seq_data.lhand.params.hand_pose.shape[1]))
    rest_hand_fullpose = np.zeros((1, seq_data.lhand.params.fullpose.shape[1]))
    ...
    lh_root_global_orientations = seq_data.lhand.params.global_orient
    lh_root_translations = seq_data.lhand.params.transl
    joints_lh_orientations = seq_data.lhand.params.fullpose

    lh_m_rest = smplx.create(model_path=cfg.model_path,
                model_type='mano',
                is_rhand = False,
                v_template = lh_vtemp,
                num_pca_comps=n_comps,
    ...
    lh_rest_params = params2torch(rest_params)
    lh_rest_output = lh_m_rest(**lh_rest_params)
    joints_lh_rest_positions = to_cpu(lh_rest_output.joints)
    joints_lh_rest_positions = joints_lh_rest_positions[0]

    # Recorded translations are relative....in the opposite direction?
    lh_root_translations += joints_lh_rest_positions[0]

    joints_hierarchy = np.array([255, 0, 1, 2, 0, 4, 5, 0, 7, 8, 0, 10, 11, 0, 13, 14], dtype=np.uint8)
    num_joints = len(joints_hierarchy)

    lh_relative_rest_configuration = np.zeros(joints_lh_rest_positions.shape, dtype=joints_lh_rest_positions.dtype)
    for ji in range(num_joints):
        parent_index = joints_hierarchy[ji]

        if parent_index == 255:
            lh_relative_rest_configuration[ji] = joints_lh_rest_positions[ji]
        else:
            lh_relative_rest_configuration[ji] = joints_lh_rest_positions[ji] - joints_lh_rest_positions[parent_index]
    ...
    num_hand_faces = lh_f.shape[0]
    num_hand_vertices = verts_lh.shape[1]
    ...
    hand_trimesh = trimesh.Trimesh(vertices=lh_vtemp, faces=lh_f)
    ...
    if is_valid:
        dumpMotion = {
            'numFrames': T,
            'handVertices': verts_lh.flatten(),
            'handJointHierarchy': joints_hierarchy.flatten(),
            'handRootOrientations': lh_root_global_orientations.flatten(),
            'handRootTranslations': lh_root_translations.flatten(),
            'handJointOrientations': joints_lh_orientations.flatten(),
            'handJointsRestConfiguration': lh_relative_rest_configuration.flatten(),
